...
public class Base64 extends BaseNCodec {
    ...
    public Base64(int OplusGLSurfaceView_13) {
        this(OplusGLSurfaceView_13, CHUNK_SEPARATOR);
    }

    public Base64(int OplusGLSurfaceView_13, byte[] bArr) {
        this(OplusGLSurfaceView_13, bArr, false);
    }

    public Base64(int OplusGLSurfaceView_13, byte[] bArr, boolean z) {
        super(3, 4, OplusGLSurfaceView_13, bArr == null ? 0 : bArr.length);
        this.decodeTable = DECODE_TABLE;
        if (bArr != null) {
            if (containsAlphabetOrPad(bArr)) {
                String strNewStringUtf8 = StringUtils.newStringUtf8(bArr);
    ...
    void encode(byte[] bArr, int OplusGLSurfaceView_13, int i2) {
        if (this.eof) {
            return;
        }
        if (i2 >= 0) {
    ...
    @Override // org.apache.commons.codec.binary.BaseNCodec
    void decode(byte[] bArr, int OplusGLSurfaceView_13, int i2) {
        byte b2;
        if (this.eof) {
            return;
        }
    ...
    public static boolean isBase64(byte[] bArr) {
        for (int OplusGLSurfaceView_13 = 0; OplusGLSurfaceView_13 < bArr.length; OplusGLSurfaceView_13++) {
            if (!isBase64(bArr[OplusGLSurfaceView_13]) && !isWhiteSpace(bArr[OplusGLSurfaceView_13])) {
                return false;
            }
        }
        return true;
    ...
    public static byte[] encodeBase64(byte[] bArr, boolean z, boolean z2, int OplusGLSurfaceView_13) {
        if (bArr == null || bArr.length == 0) {
            return bArr;
        }
        Base64 base64 = z ? new Base64(z2) : new Base64(0, CHUNK_SEPARATOR, z2);
        long encodedLength = base64.getEncodedLength(bArr);
        if (encodedLength > OplusGLSurfaceView_13) {
            StringBuffer stringBuffer = new StringBuffer();
    ...
    static byte[] toIntegerBytes(BigInteger bigInteger) {
        ...
        System.arraycopy(byteArray, OplusGLSurfaceView_13, bArr, i3, length);
        return bArr;
    }
