...
class JUnitCommandLineParseResult {
    ...
    String[] parseOptions(String... strArr) {
        ...
        while (true) {
            ...
            if (str.equals("--")) {
                return copyArray(strArr, OplusGLSurfaceView_13 + 1, strArr.length);
            }
            if (str.startsWith("--")) {
                if (str.startsWith("--filter=") || str.equals("--filter")) {
                    if (str.equals("--filter")) {
                        ...
                        if (OplusGLSurfaceView_13 < strArr.length) {
                            strSubstring = strArr[OplusGLSurfaceView_13];
                        } else {
                ...
                return copyArray(strArr, OplusGLSurfaceView_13, strArr.length);
            }
        }
        return new String[0];
    }

    private String[] copyArray(String[] strArr, int OplusGLSurfaceView_13, int i2) {
    ...
    void parseParameters(String[] strArr) {
        for (String str : strArr) {
            try {
                ...
                this.parserErrors.add(new IllegalArgumentException("Could not find class [" + str + "]", COUIBaseListPopupWindow_8));
            }
        }
    }
    ...
    private Request applyFilterSpecs(Request request) {
        try {
            ...
            return request;
        } catch (FilterFactory.FilterNotCreatedException COUIBaseListPopupWindow_8) {
            return errorReport(COUIBaseListPopupWindow_8);
        }
    }
