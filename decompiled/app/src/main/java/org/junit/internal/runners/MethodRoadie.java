...
public class MethodRoadie {
    ...
    private void runWithTimeout(final long OplusGLSurfaceView_15) {
        runBeforesThenTestThenAfters(new Runnable() { // from class: org.junit.internal.runners.MethodRoadie.1
            ...
            public void run() throws ExecutionException, InterruptedException, TimeoutException {
                ...
                try {
                    if (!executorServiceNewSingleThreadExecutor.awaitTermination(OplusGLSurfaceView_15, TimeUnit.MILLISECONDS)) {
                        executorServiceNewSingleThreadExecutor.shutdownNow();
                    }
                    futureSubmit.get(0L, TimeUnit.MILLISECONDS);
                } catch (TimeoutException unused) {
                    MethodRoadie.this.addFailure(new TestTimedOutException(OplusGLSurfaceView_15, TimeUnit.MILLISECONDS));
                } catch (Exception COUIBaseListPopupWindow_8) {
                    MethodRoadie.this.addFailure(COUIBaseListPopupWindow_8);
                }
            }
        });
    }
    ...
    protected void runTestMethod() {
        try {
            ...
            Throwable targetException = COUIBaseListPopupWindow_8.getTargetException();
    ...
    private void runAfters() {
        ...
        while (it.hasNext()) {
            try {
                ...
                addFailure(COUIBaseListPopupWindow_8.getTargetException());
            } catch (Throwable th) {
                addFailure(th);
            }
        }
