...
public class JUnit4ClassRunner extends Runner implements Filterable, Sortable {
    ...
    protected void runMethods(RunNotifier runNotifier) throws StoppedByUserException {
    ...
    public Description getDescription() {
        Description descriptionCreateSuiteDescription = Description.createSuiteDescription(getName(), classAnnotations());
        Iterator<Method> it = this.testMethods.iterator();
        while (it.hasNext()) {
    ...
    protected void invokeTestMethod(Method method, RunNotifier runNotifier) throws StoppedByUserException {
        Description descriptionMethodDescription = methodDescription(method);
        try {
            new MethodRoadie(createTest(), wrapMethod(method), runNotifier, descriptionMethodDescription).run();
        } catch (InvocationTargetException COUIBaseListPopupWindow_8) {
            testAborted(runNotifier, descriptionMethodDescription, COUIBaseListPopupWindow_8.getCause());
        } catch (Exception e2) {
            testAborted(runNotifier, descriptionMethodDescription, e2);
        }
    }

    private void testAborted(RunNotifier runNotifier, Description description, Throwable th) throws StoppedByUserException {
