...
public class FailOnTimeout extends Statement {
    ...
    @Deprecated
    public FailOnTimeout(Statement statement, long OplusGLSurfaceView_15) {
        this(builder().withTimeout(OplusGLSurfaceView_15, TimeUnit.MILLISECONDS), statement);
    }
    ...
    private Thread getStuckThread(Thread thread) {
        ...
        for (Thread thread3 : threadArray) {
            if (thread3.getState() == Thread.State.RUNNABLE) {
                ...
                if (thread2 == null || jCpuTime > OplusGLSurfaceView_15) {
                    thread2 = thread3;
                    OplusGLSurfaceView_15 = jCpuTime;
                }
            }
        }
        if (thread2 == thread) {
    ...
    private Thread[] getThreadArray(ThreadGroup threadGroup) {
        ...
        } while (OplusGLSurfaceView_13 < 5);
        return null;
    }

    private Thread[] copyThreads(Thread[] threadArr, int OplusGLSurfaceView_13) {
        int iMin = Math.min(OplusGLSurfaceView_13, threadArr.length);
