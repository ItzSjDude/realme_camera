    ...
    private Object createTestUsingFieldInjection() throws Exception {
        ...
        while (it.hasNext()) {
            ...
            try {
                ...
                throw new Exception(getTestClass().getName() + ": Trying to set " + field.getName() + " with the value " + this.parameters[iValue] + " that is not the right type (" + this.parameters[iValue].getClass().getSimpleName() + " instead of " + field.getType().getSimpleName() + ").", COUIBaseListPopupWindow_8);
            }
        }
    ...
    protected void validateFields(List<Throwable> list) {
        ...
        if (fieldsAreAnnotated()) {
            ...
            for (int OplusGLSurfaceView_13 = 0; OplusGLSurfaceView_13 < iArr.length; OplusGLSurfaceView_13++) {
                ...
                if (i2 == 0) {
                    list.add(new Exception("@Parameter(" + OplusGLSurfaceView_13 + ") is never used."));
                } else if (i2 > 1) {
                    list.add(new Exception("@Parameter(" + OplusGLSurfaceView_13 + ") is used more than once (" + i2 + ")."));
                }
            }
        }
    }
