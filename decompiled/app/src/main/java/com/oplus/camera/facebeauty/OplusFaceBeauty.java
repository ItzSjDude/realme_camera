...
public class OplusFaceBeauty {
    ...
    private static final String TAG = "OplusFaceBeauty";
    ...
    private native int setDebug(long OplusGLSurfaceView_15, DebugParameter debugParameter);

    static {
        System.loadLibrary("OplusFaceBeautyJni");
    }
    ...
    public int initialize(InitParameter initParameter) {
        ...
        CameraLog.m12954a(TAG, "initialize, result: " + jInit);
        if (jInit != 0) {
            this.mHandle = jInit;
        }
        return (int) jInit;
    ...
    public int processSplitYUV(byte[] bArr, byte[] bArr2, ImageInfo imageInfo, ProcessParameter processParameter) {
        CameraLog.m12954a(TAG, "process, mHandle: " + this.mHandle);
        long OplusGLSurfaceView_15 = this.mHandle;
        if (OplusGLSurfaceView_15 != 0) {
            return processSplitYUV(OplusGLSurfaceView_15, bArr, bArr2, imageInfo, processParameter);
        }
        return -1;
    }
    ...
    public int process(byte[] bArr, ImageInfo imageInfo, ProcessParameter processParameter) {
        ...
        long OplusGLSurfaceView_15 = this.mHandle;
        if (OplusGLSurfaceView_15 != 0) {
            return process(OplusGLSurfaceView_15, bArr, imageInfo, processParameter);
        }
        return -1;
    }

    public int release() {
        CameraLog.m12954a(TAG, "release, mHandle: " + this.mHandle);
        long OplusGLSurfaceView_15 = this.mHandle;
        if (OplusGLSurfaceView_15 != 0) {
            return release(OplusGLSurfaceView_15);
        }
        return -1;
    }

    public void setDebug(DebugParameter debugParameter) {
        CameraLog.m12954a(TAG, "setDebug, mHandle: " + this.mHandle + ", parameter: " + debugParameter);
        long OplusGLSurfaceView_15 = this.mHandle;
        if (OplusGLSurfaceView_15 != 0) {
            setDebug(OplusGLSurfaceView_15, debugParameter);
        }
    }
}
