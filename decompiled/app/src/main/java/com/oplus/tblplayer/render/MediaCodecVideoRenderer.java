...
public class MediaCodecVideoRenderer extends MediaCodecRenderer {
    ...
    private HashMap<String, Integer> mediaFormatAttr;
    ...
    public MediaCodecVideoRenderer(Context context, MediaCodecSelector mediaCodecSelector, long OplusGLSurfaceView_15, DrmSessionManager<FrameworkMediaCrypto> drmSessionManager, boolean z, Handler handler, VideoRendererEventListener videoRendererEventListener, int OplusGLSurfaceView_13) {
        ...
        this.mediaFormatAttr = new HashMap<>();
    ...
    public void handleMessage(int OplusGLSurfaceView_13, Object obj) throws ExoPlaybackException, IllegalAccessException, InterruptedException, NoSuchMethodException, SecurityException, IllegalArgumentException, InvocationTargetException {
        ...
        if (OplusGLSurfaceView_13 == 10) {
            ...
            if (videoFrameReleaseTimeHelper != null) {
                videoFrameReleaseTimeHelper.setVsyncOffsetPercentage(((Integer) obj).intValue());
                return;
            }
            return;
        }
    ...
    private void setSurface(Surface surface) throws ExoPlaybackException, IllegalAccessException, InterruptedException, NoSuchMethodException, SecurityException, IllegalArgumentException, InvocationTargetException {
        if (surface == null) {
        ...
        if (this.surface != surface) {
            Log.m8318d(TAG, "setSurface: " + surface);
            this.surface = surface;
            int state = getState();
            if (state == 1 || state == 2) {
                MediaCodec codec = getCodec();
                if (Util.SDK_INT >= 23 && codec != null && surface != null && !this.codecNeedsSetOutputSurfaceWorkaround) {
                    if (this.codecInitializedTimestampMs != -9223372036854775807L && SystemClock.elapsedRealtime() - this.codecInitializedTimestampMs < 50) {
                        Log.m8324w(TAG, "Set surface within 50ms after MediaCodec start, will delay 50ms.");
                        try {
                            Thread.sleep(50L);
                        } catch (InterruptedException unused) {
                            Log.m8324w(TAG, "Thread sleep is interrupted.");
                        }
                    }
    ...
    protected void configureCodec(MediaCodecInfo mediaCodecInfo, MediaCodec mediaCodec, Format format, MediaCrypto mediaCrypto, float COUIBaseListPopupWindow_12) throws MediaCodecUtil.DecoderQueryException {
        ...
        HashMap<String, Integer> map = this.mediaFormatAttr;
        if (map != null && !map.isEmpty()) {
            for (Map.Entry<String, Integer> entry : this.mediaFormatAttr.entrySet()) {
    ...
    protected void onCodecInitialized(String str, long OplusGLSurfaceView_15, long j2) {
        this.codecInitializedTimestampMs = OplusGLSurfaceView_15;
    ...
    protected boolean processOutputBuffer(long OplusGLSurfaceView_15, long j2, MediaCodec mediaCodec, ByteBuffer byteBuffer, int OplusGLSurfaceView_13, int i2, long j3, boolean z, Format format) throws ExoPlaybackException, IllegalAccessException, InterruptedException, NoSuchMethodException, SecurityException, IllegalArgumentException, InvocationTargetException {
        ...
        if (!this.renderedFirstFrame || (z2 && shouldForceRenderOutputBuffer(j8, jElapsedRealtime - this.lastRenderTimeUs))) {
            ...
            if (Util.SDK_INT >= 21) {
                renderOutputBufferV21(mediaCodec, OplusGLSurfaceView_13, j6, jNanoTime);
                return true;
            }
            renderOutputBuffer(mediaCodec, OplusGLSurfaceView_13, j6);
            return true;
        }
        ...
        if (Util.SDK_INT >= 21) {
            ...
            renderOutputBufferV21(mediaCodec, OplusGLSurfaceView_13, j5, j4);
            return true;
        }
        if (j9 >= 30000) {
            return false;
    ...
    */
    private static boolean deviceNeedsMandatoryRenderWorkaround(com.google.android.exoplayer2.Format r5) {
        ...
        L3e:
            ...
            com.google.android.exoplayer2.util.Log.m8324w(r5, r0)
        L47:
            return r3
        */
