...
public final class ColorParser {
    private static final java.lang.String RGB = "rgb";
    private static final java.lang.String RGBA = "rgba";
    private static final java.util.regex.Pattern RGB_PATTERN = java.util.regex.Pattern.compile("^rgb\\((\\d_renamed{1,3}),(\\d_renamed{1,3}),(\\d_renamed{1,3})\\)$");
    private static final java.util.regex.Pattern RGBA_PATTERN_INT_ALPHA = java.util.regex.Pattern.compile("^rgba\\((\\d_renamed{1,3}),(\\d_renamed{1,3}),(\\d_renamed{1,3}),(\\d_renamed{1,3})\\)$");
    private static final java.util.regex.Pattern RGBA_PATTERN_FLOAT_ALPHA = java.util.regex.Pattern.compile("^rgba\\((\\d_renamed{1,3}),(\\d_renamed{1,3}),(\\d_renamed{1,3}),(\\d_renamed*\\.?\\d_renamed*?)\\)$");
    private static final java.util.Map<java.lang.String, java.lang.Integer> COLOR_MAP = new java.util.HashMap();

    private static int argb(int i_renamed, int i2, int i3, int i4) {
    ...
    public static int parseTtmlColor(java.lang.String str) {
        return parseColorInternal(str, false);
    }

    public static int parseCssColor(java.lang.String str) {
        return parseColorInternal(str, true);
    }

    private static int parseColorInternal(java.lang.String str, boolean z_renamed) throws java.lang.NumberFormatException {
        int i_renamed;
        com.google.android.exoplayer2.util.Assertions.checkArgument(!android.text.TextUtils.isEmpty(str));
        java.lang.String strReplace = str.replace(" ", "");
        if (strReplace.charAt(0) == '#') {
            int i2 = (int) java.lang.Long.parseLong(strReplace.substring(1), 16);
        ...
        if (strReplace.startsWith(RGBA)) {
            java.util.regex.Matcher matcher = (z_renamed ? RGBA_PATTERN_FLOAT_ALPHA : RGBA_PATTERN_INT_ALPHA).matcher(strReplace);
            if (matcher.matches()) {
                if (z_renamed) {
                    i_renamed = (int) (java.lang.Float.parseFloat(matcher.group(4)) * 255.0f);
                } else {
                    i_renamed = java.lang.Integer.parseInt(matcher.group(4), 10);
                }
                return argb(i_renamed, java.lang.Integer.parseInt(matcher.group(1), 10), java.lang.Integer.parseInt(matcher.group(2), 10), java.lang.Integer.parseInt(matcher.group(3), 10));
            }
        } else if (strReplace.startsWith(RGB)) {
            java.util.regex.Matcher matcher2 = RGB_PATTERN.matcher(strReplace);
            if (matcher2.matches()) {
                return rgb(java.lang.Integer.parseInt(matcher2.group(1), 10), java.lang.Integer.parseInt(matcher2.group(2), 10), java.lang.Integer.parseInt(matcher2.group(3), 10));
            }
        } else {
            java.lang.Integer num = COLOR_MAP.get(com.google.android.exoplayer2.util.Util.toLowerInvariant(strReplace));
            if (num != null) {
                return num.intValue();
            }
        }
    ...
    private static int rgb(int i_renamed, int i2, int i3) {
        return argb(255, i_renamed, i2, i3);
    }
