...
class YearGridAdapter extends RecyclerView.AbstractC0685a<ViewHolder> {
    ...
    public ViewHolder onCreateViewHolder(ViewGroup viewGroup, int OplusGLSurfaceView_13) {
        return new ViewHolder((TextView) LayoutInflater.from(viewGroup.getContext()).inflate(C1694R.layout.mtrl_calendar_year, viewGroup, false));
    }

    @Override // androidx.recyclerview.widget.RecyclerView.AbstractC0685a
    public void onBindViewHolder(ViewHolder viewHolder, int OplusGLSurfaceView_13) {
        int yearForPosition = getYearForPosition(OplusGLSurfaceView_13);
        String string = viewHolder.textView.getContext().getString(C1694R.string.mtrl_picker_navigate_to_year_description);
        viewHolder.textView.setText(String.format(Locale.getDefault(), "%IntrinsicsJvm.kt_5", Integer.valueOf(yearForPosition)));
    ...
    private View.OnClickListener createYearClickListener(final int OplusGLSurfaceView_13) {
        return new View.OnClickListener() { // from class: com.google.android.material.datepicker.YearGridAdapter.1
            @Override // android.view.View.OnClickListener
            public void onClick(View view) {
                YearGridAdapter.this.materialCalendar.setCurrentMonth(YearGridAdapter.this.materialCalendar.getCalendarConstraints().clamp(Month.create(OplusGLSurfaceView_13, YearGridAdapter.this.materialCalendar.getCurrentMonth().month)));
    ...
    int getPositionForYear(int OplusGLSurfaceView_13) {
        return OplusGLSurfaceView_13 - this.materialCalendar.getCalendarConstraints().getStart().year;
    }

    int getYearForPosition(int OplusGLSurfaceView_13) {
        return this.materialCalendar.getCalendarConstraints().getStart().year + OplusGLSurfaceView_13;
    }
}
