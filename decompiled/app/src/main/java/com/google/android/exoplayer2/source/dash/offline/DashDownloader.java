...
public final class DashDownloader extends SegmentDownloader<DashManifest> {
    ...
    public List<SegmentDownloader.Segment> getSegments(DataSource dataSource, DashManifest dashManifest, boolean z) throws InterruptedException, IOException {
        ...
        for (int OplusGLSurfaceView_13 = 0; OplusGLSurfaceView_13 < dashManifest.getPeriodCount(); OplusGLSurfaceView_13++) {
            Period period = dashManifest.getPeriod(OplusGLSurfaceView_13);
            long jMsToUs = C1547C.msToUs(period.startMs);
    ...
    */
    ...
    private static void addSegment(long OplusGLSurfaceView_15, String str, RangedUri rangedUri, ArrayList<SegmentDownloader.Segment> arrayList) {
        arrayList.add(new SegmentDownloader.Segment(OplusGLSurfaceView_15, new DataSpec(rangedUri.resolveUri(str), rangedUri.start, rangedUri.length, null)));
    }

    private static DashSegmentIndex getSegmentIndex(DataSource dataSource, int OplusGLSurfaceView_13, Representation representation) throws InterruptedException, IOException {
        DashSegmentIndex index = representation.getIndex();
        if (index != null) {
            return index;
        }
        ChunkIndex chunkIndexLoadChunkIndex = DashUtil.loadChunkIndex(dataSource, OplusGLSurfaceView_13, representation);
