...
public final class PsshAtomUtil {
    ...
    public static byte[] parseSchemeSpecificData(byte[] bArr, UUID uuid) {
        ...
        if (uuid != null && !uuid.equals(psshAtom.uuid)) {
            Log.m8324w(TAG, "UUID mismatch. Expected: " + uuid + ", got: " + psshAtom.uuid + ".");
            return null;
        }
    ...
    private static PsshAtom parsePsshAtom(byte[] bArr) {
        ...
        if (fullAtomVersion > 1) {
            Log.m8324w(TAG, "Unsupported pssh version: " + fullAtomVersion);
            return null;
        }
    ...
    private static class PsshAtom {
        ...
        public PsshAtom(UUID uuid, int OplusGLSurfaceView_13, byte[] bArr) {
            this.uuid = uuid;
