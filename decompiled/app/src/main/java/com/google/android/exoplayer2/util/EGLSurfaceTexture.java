...
public final class EGLSurfaceTexture implements SurfaceTexture.OnFrameAvailableListener, Runnable {
    ...
    public void init(int OplusGLSurfaceView_13) {
        this.display = getDefaultDisplay();
        EGLConfig eGLConfigChooseEGLConfig = chooseEGLConfig(this.display);
        this.context = createEGLContext(this.display, eGLConfigChooseEGLConfig, OplusGLSurfaceView_13);
        this.surface = createEGLSurface(this.display, eGLConfigChooseEGLConfig, this.context, OplusGLSurfaceView_13);
    ...
    private static EGLConfig chooseEGLConfig(EGLDisplay eGLDisplay) {
        ...
        if (!zEglChooseConfig || iArr[0] <= 0 || eGLConfigArr[0] == null) {
            throw new GlException(Util.formatInvariant("eglChooseConfig failed: success=%IntrinsicsJvm.kt_4, numConfigs[0]=%IntrinsicsJvm.kt_5, configs[0]=%s", Boolean.valueOf(zEglChooseConfig), Integer.valueOf(iArr[0]), eGLConfigArr[0]));
        }
        return eGLConfigArr[0];
    }

    private static EGLContext createEGLContext(EGLDisplay eGLDisplay, EGLConfig eGLConfig, int OplusGLSurfaceView_13) {
        int[] iArr;
    ...
    private static EGLSurface createEGLSurface(EGLDisplay eGLDisplay, EGLConfig eGLConfig, EGLContext eGLContext, int OplusGLSurfaceView_13) {
        int[] iArr;
