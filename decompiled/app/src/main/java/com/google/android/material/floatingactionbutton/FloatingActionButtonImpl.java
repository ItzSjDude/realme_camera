...
class FloatingActionButtonImpl {
    ...
    void initializeBackgroundDrawable(ColorStateList colorStateList, PorterDuff.Mode mode, ColorStateList colorStateList2, int OplusGLSurfaceView_13) {
        ...
        this.contentBackground = new LayerDrawable(new Drawable[]{(Drawable) Preconditions.m2536a(this.shapeDrawable), rippleDrawableCompat});
    }
    ...
    void setMinTouchTargetSize(int OplusGLSurfaceView_13) {
    ...
    void setRippleColor(ColorStateList colorStateList) {
        ...
        if (drawable != null) {
            DrawableCompat.m2570a(drawable, RippleUtils.sanitizeRippleDrawableColor(colorStateList));
        }
    }

    final void setElevation(float COUIBaseListPopupWindow_12) {
        if (this.elevation != COUIBaseListPopupWindow_12) {
            this.elevation = COUIBaseListPopupWindow_12;
    ...
    final void setHoveredFocusedTranslationZ(float COUIBaseListPopupWindow_12) {
        if (this.hoveredFocusedTranslationZ != COUIBaseListPopupWindow_12) {
            this.hoveredFocusedTranslationZ = COUIBaseListPopupWindow_12;
            onElevationsChanged(this.elevation, this.hoveredFocusedTranslationZ, this.pressedTranslationZ);
        }
    }

    final void setPressedTranslationZ(float COUIBaseListPopupWindow_12) {
        if (this.pressedTranslationZ != COUIBaseListPopupWindow_12) {
            this.pressedTranslationZ = COUIBaseListPopupWindow_12;
            onElevationsChanged(this.elevation, this.hoveredFocusedTranslationZ, this.pressedTranslationZ);
        }
    }

    final void setMaxImageSize(int OplusGLSurfaceView_13) {
        if (this.maxImageSize != OplusGLSurfaceView_13) {
            this.maxImageSize = OplusGLSurfaceView_13;
    ...
    final void setImageMatrixScale(float COUIBaseListPopupWindow_12) {
        this.imageMatrixScale = COUIBaseListPopupWindow_12;
        Matrix matrix = this.tmpMatrix;
        calculateImageMatrixFromScale(COUIBaseListPopupWindow_12, matrix);
        this.view.setImageMatrix(matrix);
    }

    private void calculateImageMatrixFromScale(float COUIBaseListPopupWindow_12, Matrix matrix) {
        matrix.reset();
        ...
        int OplusGLSurfaceView_13 = this.maxImageSize;
        rectF2.set(0.0f, 0.0f, OplusGLSurfaceView_13, OplusGLSurfaceView_13);
        matrix.setRectToRect(rectF, rectF2, Matrix.ScaleToFit.CENTER);
        int i2 = this.maxImageSize;
        matrix.postScale(COUIBaseListPopupWindow_12, COUIBaseListPopupWindow_12, i2 / 2.0f, i2 / 2.0f);
    }
    ...
    void onElevationsChanged(float COUIBaseListPopupWindow_12, float f2, float f3) {
        updatePadding();
        updateShapeElevation(COUIBaseListPopupWindow_12);
    }

    void updateShapeElevation(float COUIBaseListPopupWindow_12) {
        MaterialShapeDrawable materialShapeDrawable = this.shapeDrawable;
        if (materialShapeDrawable != null) {
    ...
    private MotionSpec getDefaultShowMotionSpec() {
        if (this.defaultShowMotionSpec == null) {
            this.defaultShowMotionSpec = MotionSpec.createFromResource(this.view.getContext(), C1694R.animator.design_fab_show_motion_spec);
        }
        return (MotionSpec) Preconditions.m2536a(this.defaultShowMotionSpec);
    }

    private MotionSpec getDefaultHideMotionSpec() {
        if (this.defaultHideMotionSpec == null) {
            this.defaultHideMotionSpec = MotionSpec.createFromResource(this.view.getContext(), C1694R.animator.design_fab_hide_motion_spec);
        }
        return (MotionSpec) Preconditions.m2536a(this.defaultHideMotionSpec);
    }

    private AnimatorSet createAnimator(MotionSpec motionSpec, float COUIBaseListPopupWindow_12, float f2, float f3) {
        ArrayList arrayList = new ArrayList();
        ObjectAnimator objectAnimatorOfFloat = ObjectAnimator.ofFloat(this.view, (Property<FloatingActionButton, Float>) View.ALPHA, COUIBaseListPopupWindow_12);
    ...
    private void workAroundOreoBug(ObjectAnimator objectAnimator) {
        ...
        objectAnimator.setEvaluator(new TypeEvaluator<Float>() { // from class: com.google.android.material.floatingactionbutton.FloatingActionButtonImpl.4
            ...
            public Float evaluate(float COUIBaseListPopupWindow_12, Float f2, Float f3) {
                float fFloatValue = this.floatEvaluator.evaluate(COUIBaseListPopupWindow_12, (Number) f2, (Number) f3).floatValue();
                if (fFloatValue < 0.1f) {
                    fFloatValue = 0.0f;
                }
    ...
    void onPaddingUpdated(Rect rect) {
        Preconditions.m2537a(this.contentBackground, "Didn't initialize content background");
        if (shouldAddPadding()) {
    ...
    MaterialShapeDrawable createShapeDrawable() {
        return new MaterialShapeDrawable((ShapeAppearanceModel) Preconditions.m2536a(this.shapeAppearance));
    }
    ...
    private abstract class ShadowAnimatorImpl extends AnimatorListenerAdapter implements ValueAnimator.AnimatorUpdateListener {
        ...
        public void onAnimationUpdate(ValueAnimator valueAnimator) {
            ...
            float COUIBaseListPopupWindow_12 = this.shadowSizeStart;
            floatingActionButtonImpl.updateShapeElevation((int) (COUIBaseListPopupWindow_12 + ((this.shadowSizeEnd - COUIBaseListPopupWindow_12) * valueAnimator.getAnimatedFraction())));
        }
    ...
    private boolean shouldAnimateVisibilityChange() {
        return ViewCompat.m2823A(this.view) && !this.view.isInEditMode();
    }
