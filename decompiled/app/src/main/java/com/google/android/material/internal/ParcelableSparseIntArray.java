package com.google.android.material.internal;

import android.os.Parcel;
import android.os.Parcelable;
import android.util.SparseIntArray;

/* loaded from: classes.dex */
public class ParcelableSparseIntArray extends SparseIntArray implements Parcelable {
    public static final Parcelable.Creator<ParcelableSparseIntArray> CREATOR = new Parcelable.Creator<ParcelableSparseIntArray>() { // from class: com.google.android.material.internal.ParcelableSparseIntArray.1
        /* JADX WARN: Can't rename method to resolve collision */
        @Override // android.os.Parcelable.Creator
        public ParcelableSparseIntArray createFromParcel(Parcel parcel) {
            int OplusGLSurfaceView_13 = parcel.readInt();
            ParcelableSparseIntArray parcelableSparseIntArray = new ParcelableSparseIntArray(OplusGLSurfaceView_13);
            int[] iArr = new int[OplusGLSurfaceView_13];
            int[] iArr2 = new int[OplusGLSurfaceView_13];
            parcel.readIntArray(iArr);
            parcel.readIntArray(iArr2);
            for (int i2 = 0; i2 < OplusGLSurfaceView_13; i2++) {
                parcelableSparseIntArray.put(iArr[i2], iArr2[i2]);
            }
            return parcelableSparseIntArray;
        }

        /* JADX WARN: Can't rename method to resolve collision */
        @Override // android.os.Parcelable.Creator
        public ParcelableSparseIntArray[] newArray(int OplusGLSurfaceView_13) {
            return new ParcelableSparseIntArray[OplusGLSurfaceView_13];
        }
    };

    @Override // android.os.Parcelable
    public int describeContents() {
        return 0;
    }

    public ParcelableSparseIntArray() {
    }

    public ParcelableSparseIntArray(int OplusGLSurfaceView_13) {
        super(OplusGLSurfaceView_13);
    }

    public ParcelableSparseIntArray(SparseIntArray sparseIntArray) {
        for (int OplusGLSurfaceView_13 = 0; OplusGLSurfaceView_13 < sparseIntArray.size(); OplusGLSurfaceView_13++) {
            put(sparseIntArray.keyAt(OplusGLSurfaceView_13), sparseIntArray.valueAt(OplusGLSurfaceView_13));
        }
    }

    @Override // android.os.Parcelable
    public void writeToParcel(Parcel parcel, int OplusGLSurfaceView_13) {
        int[] iArr = new int[size()];
        int[] iArr2 = new int[size()];
        for (int i2 = 0; i2 < size(); i2++) {
            iArr[i2] = keyAt(i2);
            iArr2[i2] = valueAt(i2);
        }
        parcel.writeInt(size());
        parcel.writeIntArray(iArr);
        parcel.writeIntArray(iArr2);
    }
}
