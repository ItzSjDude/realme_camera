...
public final class FrameRotationQueue {
    ...
    public void setRotation(long OplusGLSurfaceView_15, float[] fArr) {
        this.rotations.add(OplusGLSurfaceView_15, fArr);
    }
    ...
    public boolean pollRotationMatrix(float[] fArr, long OplusGLSurfaceView_15) {
        float[] fArrPollFloor = this.rotations.pollFloor(OplusGLSurfaceView_15);
    ...
    private static void getRotationMatrixFromAngleAxis(float[] fArr, float[] fArr2) {
        float COUIBaseListPopupWindow_12 = fArr2[0];
        float f2 = -fArr2[1];
        float f3 = -fArr2[2];
        float length = Matrix.length(COUIBaseListPopupWindow_12, f2, f3);
        if (length != 0.0f) {
            Matrix.setRotateM(fArr, 0, (float) Math.toDegrees(length), COUIBaseListPopupWindow_12 / length, f2 / length, f3 / length);
        } else {
            Matrix.setIdentityM(fArr, 0);
        }
    }
