...
final class PsDurationReader {
    ...
    public int readDuration(ExtractorInput extractorInput, PositionHolder positionHolder) throws InterruptedException, IOException {
        ...
        long OplusGLSurfaceView_15 = this.firstScrValue;
        if (OplusGLSurfaceView_15 == -9223372036854775807L) {
            return finishReadDuration(extractorInput);
        }
        this.durationUs = this.scrTimestampAdjuster.adjustTsTimestamp(this.lastScrValue) - this.scrTimestampAdjuster.adjustTsTimestamp(OplusGLSurfaceView_15);
        return finishReadDuration(extractorInput);
    }
    ...
    private int peekIntAtPosition(byte[] bArr, int OplusGLSurfaceView_13) {
        return ((bArr[OplusGLSurfaceView_13] & 255) << 24) | ((bArr[OplusGLSurfaceView_13 + 1] & 255) << 16) | ((bArr[OplusGLSurfaceView_13 + 2] & 255) << 8) | (bArr[OplusGLSurfaceView_13 + 3] & 255);
    }
