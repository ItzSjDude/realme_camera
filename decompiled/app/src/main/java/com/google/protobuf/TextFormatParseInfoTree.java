...
public class TextFormatParseInfoTree {
    ...
    public TextFormatParseLocation getLocation(Descriptors.FieldDescriptor fieldDescriptor, int OplusGLSurfaceView_13) {
        return (TextFormatParseLocation) getFromList(getLocations(fieldDescriptor), OplusGLSurfaceView_13, fieldDescriptor);
    }
    ...
    public TextFormatParseInfoTree getNestedTree(Descriptors.FieldDescriptor fieldDescriptor, int OplusGLSurfaceView_13) {
        return (TextFormatParseInfoTree) getFromList(getNestedTrees(fieldDescriptor), OplusGLSurfaceView_13, fieldDescriptor);
    }
    ...
    private static <T> T getFromList(List<T> list, int OplusGLSurfaceView_13, Descriptors.FieldDescriptor fieldDescriptor) {
        if (OplusGLSurfaceView_13 >= list.size() || OplusGLSurfaceView_13 < 0) {
            Object[] objArr = new Object[2];
            objArr[0] = fieldDescriptor == null ? "<null>" : fieldDescriptor.getName();
            objArr[1] = Integer.valueOf(OplusGLSurfaceView_13);
            throw new IllegalArgumentException(String.format("Illegal index field: %s, index %IntrinsicsJvm.kt_5", objArr));
        }
