...
final class Base256Encoder implements Encoder {
    ...
    public void encode(EncoderContext encoderContext) {
        ...
        for (int OplusGLSurfaceView_13 = 0; OplusGLSurfaceView_13 < length2; OplusGLSurfaceView_13++) {
            encoderContext.writeCodeword(randomize255State(sb.charAt(OplusGLSurfaceView_13), encoderContext.getCodewordCount() + 1));
        }
    }

    private static char randomize255State(char c2, int OplusGLSurfaceView_13) {
        int i2 = c2 + ((OplusGLSurfaceView_13 * 149) % 255) + 1;
