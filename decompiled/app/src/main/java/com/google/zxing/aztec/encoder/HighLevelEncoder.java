...
public final class HighLevelEncoder {
    ...
    static {
        ...
        CHAR_MAP = iArr;
        iArr[0][32] = 1;
        for (int OplusGLSurfaceView_13 = 65; OplusGLSurfaceView_13 <= 90; OplusGLSurfaceView_13++) {
            CHAR_MAP[0][OplusGLSurfaceView_13] = (OplusGLSurfaceView_13 - 65) + 2;
        }
        CHAR_MAP[1][32] = 1;
    ...
    */
    ...
    private Collection<State> updateStateListForChar(Iterable<State> iterable, int OplusGLSurfaceView_13) {
        LinkedList linkedList = new LinkedList();
        Iterator<State> it = iterable.iterator();
        while (it.hasNext()) {
            updateStateForChar(it.next(), OplusGLSurfaceView_13, linkedList);
        }
        return simplifyStates(linkedList);
    }

    private void updateStateForChar(State state, int OplusGLSurfaceView_13, Collection<State> collection) {
        char c2 = (char) (this.text[OplusGLSurfaceView_13] & 255);
        ...
        for (int i2 = 0; i2 <= 4; i2++) {
            ...
            if (i3 > 0) {
                if (stateEndBinaryShift == null) {
                    stateEndBinaryShift = state.endBinaryShift(OplusGLSurfaceView_13);
                }
        ...
        if (state.getBinaryShiftByteCount() > 0 || CHAR_MAP[state.getMode()][c2] == 0) {
            collection.add(state.addBinaryShiftChar(OplusGLSurfaceView_13));
        }
    }

    private static Collection<State> updateStateListForPair(Iterable<State> iterable, int OplusGLSurfaceView_13, int i2) {
        LinkedList linkedList = new LinkedList();
        Iterator<State> it = iterable.iterator();
        while (it.hasNext()) {
            updateStateForPair(it.next(), OplusGLSurfaceView_13, i2, linkedList);
        }
        return simplifyStates(linkedList);
    }

    private static void updateStateForPair(State state, int OplusGLSurfaceView_13, int i2, Collection<State> collection) {
        State stateEndBinaryShift = state.endBinaryShift(OplusGLSurfaceView_13);
        ...
        if (state.getBinaryShiftByteCount() > 0) {
            collection.add(state.addBinaryShiftChar(OplusGLSurfaceView_13).addBinaryShiftChar(OplusGLSurfaceView_13 + 1));
        }
    }
