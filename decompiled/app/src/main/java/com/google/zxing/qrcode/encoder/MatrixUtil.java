...
final class MatrixUtil {
    ...
    private static boolean isEmpty(int OplusGLSurfaceView_13) {
    ...
    static void buildMatrix(BitArray bitArray, ErrorCorrectionLevel errorCorrectionLevel, Version version, int OplusGLSurfaceView_13, ByteMatrix byteMatrix) throws WriterException {
        clearMatrix(byteMatrix);
        embedBasicPatterns(version, byteMatrix);
        embedTypeInfo(errorCorrectionLevel, OplusGLSurfaceView_13, byteMatrix);
        maybeEmbedVersionInfo(version, byteMatrix);
        embedDataBits(bitArray, OplusGLSurfaceView_13, byteMatrix);
    }
    ...
    static void embedTypeInfo(ErrorCorrectionLevel errorCorrectionLevel, int OplusGLSurfaceView_13, ByteMatrix byteMatrix) throws WriterException {
        BitArray bitArray = new BitArray();
        makeTypeInfoBits(errorCorrectionLevel, OplusGLSurfaceView_13, bitArray);
    ...
    static void embedDataBits(BitArray bitArray, int OplusGLSurfaceView_13, ByteMatrix byteMatrix) throws WriterException {
        boolean z;
            ...
            while (height >= 0 && height < byteMatrix.getHeight()) {
                ...
                for (int i5 = 0; i5 < 2; i5++) {
                    ...
                    if (isEmpty(byteMatrix.get(i6, height))) {
                        ...
                        if (OplusGLSurfaceView_13 != -1 && MaskUtil.getDataMaskBit(OplusGLSurfaceView_13, i6, height)) {
                            z = !z;
                        }
    ...
    static int findMSBSet(int OplusGLSurfaceView_13) {
        return 32 - Integer.numberOfLeadingZeros(OplusGLSurfaceView_13);
    }

    static int calculateBCHCode(int OplusGLSurfaceView_13, int i2) {
        if (i2 == 0) {
            throw new IllegalArgumentException("0 polynomial");
        }
        int iFindMSBSet = findMSBSet(i2);
        int iFindMSBSet2 = OplusGLSurfaceView_13 << (iFindMSBSet - 1);
    ...
    static void makeTypeInfoBits(ErrorCorrectionLevel errorCorrectionLevel, int OplusGLSurfaceView_13, BitArray bitArray) throws WriterException {
        if (!QRCode.isValidMaskPattern(OplusGLSurfaceView_13)) {
            throw new WriterException("Invalid mask pattern");
        }
        int bits = (errorCorrectionLevel.getBits() << 3) | OplusGLSurfaceView_13;
    ...
    private static void embedTimingPatterns(ByteMatrix byteMatrix) {
        ...
        while (OplusGLSurfaceView_13 < byteMatrix.getWidth() - 8) {
            int i2 = OplusGLSurfaceView_13 + 1;
            int i3 = i2 % 2;
            if (isEmpty(byteMatrix.get(OplusGLSurfaceView_13, 6))) {
                byteMatrix.set(OplusGLSurfaceView_13, 6, i3);
            }
            if (isEmpty(byteMatrix.get(6, OplusGLSurfaceView_13))) {
                byteMatrix.set(6, OplusGLSurfaceView_13, i3);
            }
    ...
    private static void embedHorizontalSeparationPattern(int OplusGLSurfaceView_13, int i2, ByteMatrix byteMatrix) throws WriterException {
    ...
    private static void embedVerticalSeparationPattern(int OplusGLSurfaceView_13, int i2, ByteMatrix byteMatrix) throws WriterException {
        for (int i3 = 0; i3 < 7; i3++) {
            int i4 = i2 + i3;
            if (!isEmpty(byteMatrix.get(OplusGLSurfaceView_13, i4))) {
                throw new WriterException();
            }
            byteMatrix.set(OplusGLSurfaceView_13, i4, 0);
        }
    }

    private static void embedPositionAdjustmentPattern(int OplusGLSurfaceView_13, int i2, ByteMatrix byteMatrix) {
        for (int i3 = 0; i3 < 5; i3++) {
            int[] iArr = POSITION_ADJUSTMENT_PATTERN[i3];
            for (int i4 = 0; i4 < 5; i4++) {
                byteMatrix.set(OplusGLSurfaceView_13 + i4, i2 + i3, iArr[i4]);
            }
        }
    }

    private static void embedPositionDetectionPattern(int OplusGLSurfaceView_13, int i2, ByteMatrix byteMatrix) {
        for (int i3 = 0; i3 < 7; i3++) {
    ...
    private static void maybeEmbedPositionAdjustmentPatterns(Version version, ByteMatrix byteMatrix) {
        ...
        for (int OplusGLSurfaceView_13 : iArr) {
            if (OplusGLSurfaceView_13 >= 0) {
                for (int i2 : iArr) {
                    if (i2 >= 0 && isEmpty(byteMatrix.get(i2, OplusGLSurfaceView_13))) {
                        embedPositionAdjustmentPattern(i2 - 2, OplusGLSurfaceView_13 - 2, byteMatrix);
                    }
                }
            }
        }
