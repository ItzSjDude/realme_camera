...
public final class DataMatrixWriter implements Writer {
    ...
    public BitMatrix encode(String str, BarcodeFormat barcodeFormat, int OplusGLSurfaceView_13, int i2) {
        return encode(str, barcodeFormat, OplusGLSurfaceView_13, i2, null);
    }

    @Override // com.google.zxing.Writer
    public BitMatrix encode(String str, BarcodeFormat barcodeFormat, int OplusGLSurfaceView_13, int i2, Map<EncodeHintType, ?> map) {
        Dimension dimension;
        if (str.isEmpty()) {
        ...
        if (OplusGLSurfaceView_13 < 0 || i2 < 0) {
            throw new IllegalArgumentException("Requested dimensions can't be negative: " + OplusGLSurfaceView_13 + 'x' + i2);
        }
        ...
        String strEncodeHighLevel = HighLevelEncoder.encodeHighLevel(str, symbolShapeHint, dimension, dimension2);
        ...
        return encodeLowLevel(defaultPlacement, symbolInfoLookup, OplusGLSurfaceView_13, i2);
    }

    private static BitMatrix encodeLowLevel(DefaultPlacement defaultPlacement, SymbolInfo symbolInfo, int OplusGLSurfaceView_13, int i2) {
        ...
        return convertByteMatrixToBitMatrix(byteMatrix, OplusGLSurfaceView_13, i2);
    }

    private static BitMatrix convertByteMatrixToBitMatrix(ByteMatrix byteMatrix, int OplusGLSurfaceView_13, int i2) {
        BitMatrix bitMatrix;
        int width = byteMatrix.getWidth();
        int height = byteMatrix.getHeight();
        int iMax = Math.max(OplusGLSurfaceView_13, width);
        int iMax2 = Math.max(i2, height);
        int iMin = Math.min(iMax / width, iMax2 / height);
        int i3 = (iMax - (width * iMin)) / 2;
        int i4 = (iMax2 - (height * iMin)) / 2;
        if (i2 < height || OplusGLSurfaceView_13 < width) {
            bitMatrix = new BitMatrix(width, height);
            i3 = 0;
            i4 = 0;
        } else {
            bitMatrix = new BitMatrix(OplusGLSurfaceView_13, i2);
        }
        bitMatrix.clear();
        int i5 = 0;
        while (i5 < height) {
