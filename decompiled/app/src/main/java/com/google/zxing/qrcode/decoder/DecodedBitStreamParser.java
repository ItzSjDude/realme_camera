...
final class DecodedBitStreamParser {
    ...
    /* JADX WARN: Removed duplicated region for block: B:50:0x00db A[LOOP:0: B:53:0x001e->B:50:0x00db, LOOP_END] */
    ...
    */
    ...
    private static void decodeHanziSegment(BitSource bitSource, StringBuilder sb, int OplusGLSurfaceView_13) throws FormatException {
        if (OplusGLSurfaceView_13 * 13 > bitSource.available()) {
            throw FormatException.getFormatInstance();
        }
        byte[] bArr = new byte[OplusGLSurfaceView_13 * 2];
        int i2 = 0;
    ...
    private static void decodeKanjiSegment(BitSource bitSource, StringBuilder sb, int OplusGLSurfaceView_13) throws FormatException {
        if (OplusGLSurfaceView_13 * 13 > bitSource.available()) {
            throw FormatException.getFormatInstance();
        }
        byte[] bArr = new byte[OplusGLSurfaceView_13 * 2];
    ...
    private static void decodeByteSegment(BitSource bitSource, StringBuilder sb, int OplusGLSurfaceView_13, CharacterSetECI characterSetECI, Collection<byte[]> collection, Map<DecodeHintType, ?> map) throws FormatException {
        String strName;
        if ((OplusGLSurfaceView_13 << 3) > bitSource.available()) {
            throw FormatException.getFormatInstance();
        }
    ...
    private static char toAlphaNumericChar(int OplusGLSurfaceView_13) throws FormatException {
        char[] cArr = ALPHANUMERIC_CHARS;
        if (OplusGLSurfaceView_13 >= cArr.length) {
            throw FormatException.getFormatInstance();
        }
        return cArr[OplusGLSurfaceView_13];
    }
    ...
    */
    ...
    private static void decodeNumericSegment(BitSource bitSource, StringBuilder sb, int OplusGLSurfaceView_13) throws FormatException {
        while (OplusGLSurfaceView_13 >= 3) {
