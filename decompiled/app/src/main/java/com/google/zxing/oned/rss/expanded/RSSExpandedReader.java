...
public final class RSSExpandedReader extends AbstractRSSReader {
    ...
    public Result decodeRow(int OplusGLSurfaceView_13, BitArray bitArray, Map<DecodeHintType, ?> map) throws NotFoundException, FormatException {
        this.pairs.clear();
        this.startFromEven = false;
        try {
            return constructResult(decodeRow2pairs(OplusGLSurfaceView_13, bitArray));
        } catch (NotFoundException unused) {
            this.pairs.clear();
            this.startFromEven = true;
            return constructResult(this.decodeRow2pairs(OplusGLSurfaceView_13, bitArray));
        }
    }
    ...
    List<ExpandedPair> decodeRow2pairs(int OplusGLSurfaceView_13, BitArray bitArray) throws NotFoundException {
        boolean z = false;
        while (!z) {
            try {
                this.pairs.add(retrieveNextPair(bitArray, this.pairs, OplusGLSurfaceView_13));
        ...
        storeRow(OplusGLSurfaceView_13, false);
        if (z2) {
    ...
    private List<ExpandedPair> checkRows(List<ExpandedRow> list, int OplusGLSurfaceView_13) throws NotFoundException {
        while (OplusGLSurfaceView_13 < this.rows.size()) {
            ExpandedRow expandedRow = this.rows.get(OplusGLSurfaceView_13);
            this.pairs.clear();
            ...
            if (isValidSequence(this.pairs)) {
                ...
                try {
                    return this.checkRows(arrayList, OplusGLSurfaceView_13 + 1);
                } catch (NotFoundException unused) {
                    continue;
                }
            }
        ...
        for (int[] iArr : FINDER_PATTERN_SEQUENCES) {
            if (list.size() <= iArr.length) {
                ...
                while (true) {
                    ...
                    if (list.get(OplusGLSurfaceView_13).getFinderPattern().getValue() != iArr[OplusGLSurfaceView_13]) {
                        z = false;
                        break;
                    }
                    OplusGLSurfaceView_13++;
    ...
    private void storeRow(int OplusGLSurfaceView_13, boolean z) {
        ...
        while (true) {
            ...
            if (expandedRow.getRowNumber() > OplusGLSurfaceView_13) {
        ...
        this.rows.add(i2, new ExpandedRow(this.pairs, OplusGLSurfaceView_13, z));
    ...
    private boolean checkChecksum() {
        ...
        return ((OplusGLSurfaceView_13 + (-4)) * 211) + (checksumPortion % 211) == leftChar.getValue();
    }

    private static int getNextSecondBar(BitArray bitArray, int OplusGLSurfaceView_13) {
        if (bitArray.get(OplusGLSurfaceView_13)) {
            return bitArray.getNextSet(bitArray.getNextUnset(OplusGLSurfaceView_13));
        }
        return bitArray.getNextUnset(bitArray.getNextSet(OplusGLSurfaceView_13));
    }

    ExpandedPair retrieveNextPair(BitArray bitArray, List<ExpandedPair> list, int OplusGLSurfaceView_13) throws NotFoundException {
        ...
        do {
            ...
            foundFinderPattern = parseFoundFinderPattern(bitArray, OplusGLSurfaceView_13, z);
    ...
    private void findNextPair(BitArray bitArray, List<ExpandedPair> list, int OplusGLSurfaceView_13) throws NotFoundException {
        ...
        if (OplusGLSurfaceView_13 < 0) {
            OplusGLSurfaceView_13 = list.isEmpty() ? 0 : list.get(list.size() - 1).getFinderPattern().getStartEnd()[1];
        }
        boolean z = list.size() % 2 != 0;
        if (this.startFromEven) {
            z = !z;
    ...
    private static void reverseCounters(int[] iArr) {
        ...
        for (int OplusGLSurfaceView_13 = 0; OplusGLSurfaceView_13 < length / 2; OplusGLSurfaceView_13++) {
            int i2 = iArr[OplusGLSurfaceView_13];
            int i3 = (length - OplusGLSurfaceView_13) - 1;
            iArr[OplusGLSurfaceView_13] = iArr[i3];
            iArr[i3] = i2;
        }
    }

    private FinderPattern parseFoundFinderPattern(BitArray bitArray, int OplusGLSurfaceView_13, boolean z) {
        int i2;
        int i3;
        int i4;
        if (z) {
        ...
        try {
            return new FinderPattern(parseFinderValue(decodeFinderCounters, FINDER_PATTERNS), new int[]{i4, i3}, i4, i3, OplusGLSurfaceView_13);
        } catch (NotFoundException unused) {
            return null;
        }
    }
    ...
    DataCharacter decodeDataCharacter(BitArray bitArray, FinderPattern finderPattern, boolean z, boolean z2) throws NotFoundException {
        ...
        float COUIBaseListPopupWindow_12 = (finderPattern.getStartEnd()[1] - finderPattern.getStartEnd()[0]) / 15.0f;
        if (Math.abs(fSum - COUIBaseListPopupWindow_12) / COUIBaseListPopupWindow_12 > 0.3f) {
            throw NotFoundException.getNotFoundInstance();
        }
    ...
    private void adjustOddEvenCounts(int OplusGLSurfaceView_13) throws NotFoundException {
        boolean z;
        boolean z2;
        boolean z3;
        boolean z4;
