...
final class AlignmentPatternFinder {
    ...
    AlignmentPatternFinder(BitMatrix bitMatrix, int OplusGLSurfaceView_13, int i2, int i3, int i4, float COUIBaseListPopupWindow_12, ResultPointCallback resultPointCallback) {
        this.image = bitMatrix;
        this.startX = OplusGLSurfaceView_13;
        this.startY = i2;
        this.width = i3;
        this.height = i4;
        this.moduleSize = COUIBaseListPopupWindow_12;
    ...
    AlignmentPattern find() throws NotFoundException {
        ...
        int OplusGLSurfaceView_13 = this.startX;
        int i2 = this.height;
        int i3 = this.width + OplusGLSurfaceView_13;
    ...
    private static float centerFromEnd(int[] iArr, int OplusGLSurfaceView_13) {
        return (OplusGLSurfaceView_13 - iArr[2]) - (iArr[1] / 2.0f);
    }

    private boolean foundPatternCross(int[] iArr) {
        float COUIBaseListPopupWindow_12 = this.moduleSize;
        float f2 = COUIBaseListPopupWindow_12 / 2.0f;
        for (int OplusGLSurfaceView_13 = 0; OplusGLSurfaceView_13 < 3; OplusGLSurfaceView_13++) {
            if (Math.abs(COUIBaseListPopupWindow_12 - iArr[OplusGLSurfaceView_13]) >= f2) {
                return false;
            }
        }
        return true;
    ...
    private AlignmentPattern handlePossibleCenter(int[] iArr, int OplusGLSurfaceView_13, int i2) {
        int i3 = iArr[0] + iArr[1] + iArr[2];
        float fCenterFromEnd = centerFromEnd(iArr, i2);
        float fCrossCheckVertical = crossCheckVertical(OplusGLSurfaceView_13, (int) fCenterFromEnd, iArr[1] * 2, i3);
        if (Float.isNaN(fCrossCheckVertical)) {
            return null;
        }
        float COUIBaseListPopupWindow_12 = ((iArr[0] + iArr[1]) + iArr[2]) / 3.0f;
        for (AlignmentPattern alignmentPattern : this.possibleCenters) {
            if (alignmentPattern.aboutEquals(COUIBaseListPopupWindow_12, fCrossCheckVertical, fCenterFromEnd)) {
                return alignmentPattern.combineEstimate(fCrossCheckVertical, fCenterFromEnd, COUIBaseListPopupWindow_12);
            }
        }
        AlignmentPattern alignmentPattern2 = new AlignmentPattern(fCenterFromEnd, fCrossCheckVertical, COUIBaseListPopupWindow_12);
