...
public class QRCodeReader implements Reader {
    ...
    private static BitMatrix extractPureBits(BitMatrix bitMatrix) throws NotFoundException {
        ...
        int OplusGLSurfaceView_13 = topLeftOnBit[1];
        int i2 = bottomRightOnBit[1];
        int i3 = topLeftOnBit[0];
        int i4 = bottomRightOnBit[0];
        if (i3 >= i4 || OplusGLSurfaceView_13 >= i2) {
            throw NotFoundException.getNotFoundInstance();
        }
    ...
    private static float moduleSize(int[] iArr, BitMatrix bitMatrix) throws NotFoundException {
        ...
        int i2 = iArr[1];
        int i3 = 0;
        while (OplusGLSurfaceView_13 < width && i2 < height) {
            if (z != bitMatrix.get(OplusGLSurfaceView_13, i2)) {
                i3++;
                if (i3 == 5) {
                    break;
                }
        ...
        if (OplusGLSurfaceView_13 == width || i2 == height) {
            throw NotFoundException.getNotFoundInstance();
        }
        return (OplusGLSurfaceView_13 - iArr[0]) / 7.0f;
    }
}
