...
final class DetectionResultRowIndicatorColumn extends DetectionResultColumn {
    ...
    BarcodeMetadata getBarcodeMetadata() {
        ...
        for (Codeword codeword : codewords) {
            if (codeword != null) {
                ...
                int OplusGLSurfaceView_13 = rowNumber % 3;
    ...
    private void removeIncorrectCodewords(Codeword[] codewordArr, BarcodeMetadata barcodeMetadata) {
        for (int OplusGLSurfaceView_13 = 0; OplusGLSurfaceView_13 < codewordArr.length; OplusGLSurfaceView_13++) {
            Codeword codeword = codewordArr[OplusGLSurfaceView_13];
            if (codewordArr[OplusGLSurfaceView_13] != null) {
                int value = codeword.getValue() % 30;
                int rowNumber = codeword.getRowNumber();
                if (rowNumber > barcodeMetadata.getRowCount()) {
                    codewordArr[OplusGLSurfaceView_13] = null;
                } else {
                    if (!this.isLeft) {
                        rowNumber += 2;
                    }
    ...
    public String toString() {
        return "IsLeft: " + this.isLeft + '\OplusGLSurfaceView_11' + super.toString();
    }
}
