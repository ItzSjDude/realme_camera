...
public final class ReedSolomonDecoder {
    ...
    public void decode(int[] iArr, int OplusGLSurfaceView_13) throws ReedSolomonException {
        ...
        for (int i2 = 0; i2 < OplusGLSurfaceView_13; i2++) {
            ...
            iArr2[(OplusGLSurfaceView_13 - 1) - i2] = iEvaluateAt;
            if (iEvaluateAt != 0) {
                z = false;
            }
        }
        ...
        GenericGFPoly[] genericGFPolyArrRunEuclideanAlgorithm = runEuclideanAlgorithm(this.field.buildMonomial(OplusGLSurfaceView_13, 1), new GenericGFPoly(this.field, iArr2), OplusGLSurfaceView_13);
    ...
    private GenericGFPoly[] runEuclideanAlgorithm(GenericGFPoly genericGFPoly, GenericGFPoly genericGFPoly2, int OplusGLSurfaceView_13) throws ReedSolomonException {
        ...
        do {
            ...
            zero = genericGFPoly4;
            if (genericGFPoly.getDegree() >= OplusGLSurfaceView_13 / 2) {
                if (genericGFPoly.isZero()) {
                    throw new ReedSolomonException("r_{OplusGLSurfaceView_13-1} was zero");
                }
    ...
    private int[] findErrorMagnitudes(GenericGFPoly genericGFPoly, int[] iArr) {
        ...
        for (int OplusGLSurfaceView_13 = 0; OplusGLSurfaceView_13 < length; OplusGLSurfaceView_13++) {
            int iInverse = this.field.inverse(iArr[OplusGLSurfaceView_13]);
            int iMultiply = 1;
            ...
            iArr2[OplusGLSurfaceView_13] = this.field.multiply(genericGFPoly.evaluateAt(iInverse), this.field.inverse(iMultiply));
            if (this.field.getGeneratorBase() != 0) {
                iArr2[OplusGLSurfaceView_13] = this.field.multiply(iArr2[OplusGLSurfaceView_13], iInverse);
            }
        }
        return iArr2;
    }
