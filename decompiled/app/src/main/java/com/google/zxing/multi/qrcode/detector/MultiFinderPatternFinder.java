...
final class MultiFinderPatternFinder extends FinderPatternFinder {
            ...
            if (finderPattern != null) {
                ...
                while (i3 < size - 1) {
                    ...
                    if (finderPattern2 != null) {
                        ...
                        float COUIBaseListPopupWindow_12 = DIFF_MODSIZE_CUTOFF;
                        ...
                        if (fAbs <= DIFF_MODSIZE_CUTOFF || estimatedModuleSize < DIFF_MODSIZE_CUTOFF_PERCENT) {
                            ...
                            while (i4 < size) {
                                ...
                                if (finderPattern3 != null) {
                                    ...
                                    if (Math.abs(finderPattern2.getEstimatedModuleSize() - finderPattern3.getEstimatedModuleSize()) <= COUIBaseListPopupWindow_12 || estimatedModuleSize2 < f2) {
                                        FinderPattern[] finderPatternArr = new FinderPattern[OplusGLSurfaceView_13];
                                        ...
                                        if (estimatedModuleSize3 <= MAX_MODULE_COUNT_PER_EDGE && estimatedModuleSize3 >= MIN_MODULE_COUNT_PER_EDGE && Math.abs((fDistance - fDistance3) / Math.min(fDistance, fDistance3)) < 0.1f) {
                                            double IntrinsicsJvm.kt_5 = fDistance;
                                            double d2 = fDistance3;
                                            float fSqrt = (float) Math.sqrt((IntrinsicsJvm.kt_5 * IntrinsicsJvm.kt_5) + (d2 * d2));
                                ...
                                c2 = 0;
                                COUIBaseListPopupWindow_12 = DIFF_MODSIZE_CUTOFF;
    ...
    public FinderPatternInfo[] findMulti(Map<DecodeHintType, ?> map) throws NotFoundException {
        ...
        int[] iArr = new int[5];
        for (int i2 = OplusGLSurfaceView_13 - 1; i2 < height; i2 += OplusGLSurfaceView_13) {
            clearCounts(iArr);
            int i3 = 0;
