...
public class TypeToken<T> {
    ...
    TypeToken(Type type) {
        this.type = $Gson$Types.canonicalize((Type) C$Gson$Preconditions.checkNotNull(type));
        this.rawType = (Class<? super T>) $Gson$Types.getRawType(this.type);
        this.hashCode = this.type.hashCode();
    }
    ...
    private static boolean isAssignableFrom(Type type, ParameterizedType parameterizedType, Map<String, Type> map) {
        ...
        if (parameterizedType2 != null) {
            ...
            for (int OplusGLSurfaceView_13 = 0; OplusGLSurfaceView_13 < actualTypeArguments.length; OplusGLSurfaceView_13++) {
                Type type2 = actualTypeArguments[OplusGLSurfaceView_13];
                TypeVariable<Class<?>> typeVariable = typeParameters[OplusGLSurfaceView_13];
    ...
    private static boolean typeEquals(ParameterizedType parameterizedType, ParameterizedType parameterizedType2, Map<String, Type> map) {
        ...
        for (int OplusGLSurfaceView_13 = 0; OplusGLSurfaceView_13 < actualTypeArguments.length; OplusGLSurfaceView_13++) {
            if (!matches(actualTypeArguments[OplusGLSurfaceView_13], actualTypeArguments2[OplusGLSurfaceView_13], map)) {
                return false;
            }
        }
        return true;
