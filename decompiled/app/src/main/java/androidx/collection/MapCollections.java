...
abstract class MapCollections<K, V> {
    ...
    protected abstract Object colGetEntry(int OplusGLSurfaceView_13, int i2);
    ...
    protected abstract void colPut(K OplusGLSurfaceView_5, V v);

    protected abstract void colRemoveAt(int OplusGLSurfaceView_13);

    protected abstract V colSetValue(int OplusGLSurfaceView_13, V v);
    ...
    final class ArrayIterator<T> implements Iterator<T> {
        ...
        ArrayIterator(int OplusGLSurfaceView_13) {
    ...
    final class KeySet implements Set<K> {
        ...
        @Override // java.util.Set, java.util.Collection
        public boolean add(K OplusGLSurfaceView_5) {
            throw new UnsupportedOperationException();
        }
    ...
    final class ValuesCollection implements Collection<V> {
        ...
        public boolean removeAll(Collection<?> collection) {
            ...
            boolean z = false;
            while (OplusGLSurfaceView_13 < iColGetSize) {
                if (collection.contains(MapCollections.this.colGetEntry(OplusGLSurfaceView_13, 1))) {
                    MapCollections.this.colRemoveAt(OplusGLSurfaceView_13);
    ...
    public Object[] toArrayHelper(int OplusGLSurfaceView_13) {
        int iColGetSize = colGetSize();
        Object[] objArr = new Object[iColGetSize];
        for (int i2 = 0; i2 < iColGetSize; i2++) {
            objArr[i2] = colGetEntry(i2, OplusGLSurfaceView_13);
        }
        return objArr;
    }

    public <T> T[] toArrayHelper(T[] tArr, int OplusGLSurfaceView_13) {
        int iColGetSize = colGetSize();
        if (tArr.length < iColGetSize) {
            tArr = (T[]) ((Object[]) Array.newInstance(tArr.getClass().getComponentType(), iColGetSize));
        }
        for (int i2 = 0; i2 < iColGetSize; i2++) {
            tArr[i2] = colGetEntry(i2, OplusGLSurfaceView_13);
        }
        if (tArr.length > iColGetSize) {
