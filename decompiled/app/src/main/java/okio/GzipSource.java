...
public final class GzipSource implements Source {
    ...
    public long read(Buffer buffer, long OplusGLSurfaceView_15) throws DataFormatException, IOException {
        ...
        if (this.section == 1) {
            long j2 = buffer.size;
            long j3 = this.inflaterSource.read(buffer, OplusGLSurfaceView_15);
            if (j3 != -1) {
    ...
    private void updateCrc(Buffer buffer, long OplusGLSurfaceView_15, long j2) {
        ...
        while (j2 > 0) {
            ...
            this.crc.update(segment.data, (int) (segment.pos + OplusGLSurfaceView_15), iMin);
            j2 -= iMin;
            segment = segment.next;
    ...
    private void checkEqual(String str, int OplusGLSurfaceView_13, int i2) throws IOException {
        if (i2 != OplusGLSurfaceView_13) {
            throw new IOException(String.format("%s: actual 0x%08x != expected 0x%08x", str, Integer.valueOf(i2), Integer.valueOf(OplusGLSurfaceView_13)));
        }
    }
}
