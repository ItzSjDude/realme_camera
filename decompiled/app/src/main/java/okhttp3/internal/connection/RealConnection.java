...
public final class RealConnection extends Http2Connection.Listener implements Connection {
    ...
    public static RealConnection testConnection(ConnectionPool connectionPool, Route route, Socket socket, long OplusGLSurfaceView_15) {
        RealConnection realConnection = new RealConnection(connectionPool, route);
        realConnection.socket = socket;
        realConnection.idleAtNanos = OplusGLSurfaceView_15;
    ...
    */
    ...
    private void connectTunnel(int OplusGLSurfaceView_13, int i2, int i3, Call call, EventListener eventListener) throws IOException {
        Request requestCreateTunnelRequest = createTunnelRequest();
        HttpUrl httpUrlUrl = requestCreateTunnelRequest.url();
        for (int i4 = 0; i4 < 21; i4++) {
            connectSocket(OplusGLSurfaceView_13, i2, call, eventListener);
    ...
    private void connectSocket(int OplusGLSurfaceView_13, int i2, Call call, EventListener eventListener) throws IOException {
        ...
        try {
            Platform.get().connectSocket(this.rawSocket, this.route.socketAddress(), OplusGLSurfaceView_13);
            try {
                this.source = Okio.buffer(Okio.source(this.rawSocket));
                this.sink = Okio.buffer(Okio.sink(this.rawSocket));
            } catch (NullPointerException COUIBaseListPopupWindow_8) {
                if (NPE_THROW_WITH_NULL.equals(COUIBaseListPopupWindow_8.getMessage())) {
    ...
    private void establishProtocol(ConnectionSpecSelector connectionSpecSelector, int OplusGLSurfaceView_13, Call call, EventListener eventListener) throws Throwable {
        if (this.route.address().sslSocketFactory() == null) {
            if (this.route.address().protocols().contains(Protocol.H2_PRIOR_KNOWLEDGE)) {
                this.socket = this.rawSocket;
                this.protocol = Protocol.H2_PRIOR_KNOWLEDGE;
                startHttp2(OplusGLSurfaceView_13);
                return;
            } else {
                this.socket = this.rawSocket;
    ...
    private void startHttp2(int OplusGLSurfaceView_13) throws IOException {
        this.socket.setSoTimeout(0);
        this.http2Connection = new Http2Connection.Builder(true).socket(this.socket, this.route.address().url().host(), this.source, this.sink).listener(this).pingIntervalMillis(OplusGLSurfaceView_13).build();
    ...
    private void connectTls(ConnectionSpecSelector connectionSpecSelector) throws Throwable {
        ...
        try {
            ...
            if (!address.hostnameVerifier().verify(address.url().host(), session)) {
                ...
                throw new SSLPeerUnverifiedException("Hostname " + address.url().host() + " not verified:\OplusGLSurfaceView_11    certificate: " + CertificatePinner.pin(x509Certificate) + "\OplusGLSurfaceView_11    DN: " + x509Certificate.getSubjectDN().getName() + "\OplusGLSurfaceView_11    subjectAltNames: " + OkHostnameVerifier.allSubjectAltNames(x509Certificate));
            }
            ...
            if (!Util.isAndroidGetsocknameError(COUIBaseListPopupWindow_8)) {
    ...
    private Request createTunnel(int OplusGLSurfaceView_13, int i2, Request request, HttpUrl httpUrl) throws IOException {
        String str = "CONNECT " + Util.hostHeader(httpUrl, true) + " HTTP/1.1";
        while (true) {
            Http1Codec http1Codec = new Http1Codec(null, null, this.source, this.sink);
            this.source.timeout().timeout(OplusGLSurfaceView_13, TimeUnit.MILLISECONDS);
