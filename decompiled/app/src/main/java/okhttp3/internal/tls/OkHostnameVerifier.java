...
public final class OkHostnameVerifier implements HostnameVerifier {
    ...
    private boolean verifyIpAddress(String str, X509Certificate x509Certificate) throws CertificateParsingException {
        ...
        for (int OplusGLSurfaceView_13 = 0; OplusGLSurfaceView_13 < size; OplusGLSurfaceView_13++) {
            if (str.equalsIgnoreCase(subjectAltNames.get(OplusGLSurfaceView_13))) {
                return true;
            }
        }
        return false;
    ...
    private static List<String> getSubjectAltNames(X509Certificate x509Certificate, int OplusGLSurfaceView_13) throws CertificateParsingException {
        Integer num;
        String str;
        ...
        try {
            ...
            for (List<?> list : subjectAlternativeNames) {
                if (list != null && list.size() >= 2 && (num = (Integer) list.get(0)) != null && num.intValue() == OplusGLSurfaceView_13 && (str = (String) list.get(1)) != null) {
                    arrayList.add(str);
                }
            }
            return arrayList;
