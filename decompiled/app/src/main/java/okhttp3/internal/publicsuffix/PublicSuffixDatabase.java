...
public final class PublicSuffixDatabase {
    ...
    public String getEffectiveTldPlusOne(String str) throws InterruptedException {
        ...
        for (int OplusGLSurfaceView_13 = length - length2; OplusGLSurfaceView_13 < strArrSplit2.length; OplusGLSurfaceView_13++) {
            sb.append(strArrSplit2[OplusGLSurfaceView_13]);
            sb.append('.');
        }
    ...
    private String[] findMatchingRule(String[] strArr) throws InterruptedException {
        ...
        for (int OplusGLSurfaceView_13 = 0; OplusGLSurfaceView_13 < strArr.length; OplusGLSurfaceView_13++) {
            bArr[OplusGLSurfaceView_13] = strArr[OplusGLSurfaceView_13].getBytes(Util.UTF_8);
        }
        int i2 = 0;
        while (true) {
            if (i2 >= bArr.length) {
    ...
    private static String binarySearchBytes(byte[] bArr, byte[][] bArr2, int OplusGLSurfaceView_13) {
        int i2;
        int i3;
        int i4;
        ...
        while (true) {
            try {
                try {
                    ...
                    Platform.get().log(5, "Failed to read public suffix list", COUIBaseListPopupWindow_8);
                    if (z) {
                        Thread.currentThread().interrupt();
                        return;
                    }
