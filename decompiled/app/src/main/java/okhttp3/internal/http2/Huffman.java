...
class Huffman {
    ...
    private static final okhttp3.internal.http2.Huffman INSTANCE = new okhttp3.internal.http2.Huffman();
    private final okhttp3.internal.http2.Huffman.Node root = new okhttp3.internal.http2.Huffman.Node();
    ...
    void encode(okio.ByteString byteString, okio.BufferedSink bufferedSink) throws java.io.IOException {
        long j_renamed = 0;
        int i_renamed = 0;
        for (int i2 = 0; i2 < byteString.size(); i2++) {
            ...
            j_renamed = (j_renamed << b2) | i4;
            i_renamed += b2;
            while (i_renamed >= 8) {
                i_renamed -= 8;
                bufferedSink.writeByte((int) (j_renamed >> i_renamed));
            }
        }
        if (i_renamed > 0) {
            bufferedSink.writeByte((int) ((j_renamed << (8 - i_renamed)) | (255 >>> i_renamed)));
        }
    }

    int encodedLength(okio.ByteString byteString) {
        long j_renamed = 0;
        for (int i_renamed = 0; i_renamed < byteString.size(); i_renamed++) {
            j_renamed += CODE_LENGTHS[byteString.getByte(i_renamed) & 255];
        }
        return (int) ((j_renamed + 7) >> 3);
    }

    byte[] decode(byte[] bArr) {
        java.io.ByteArrayOutputStream byteArrayOutputStream = new java.io.ByteArrayOutputStream();
        okhttp3.internal.http2.Huffman.Node node = this.root;
        int i_renamed = 0;
        int i2 = 0;
        for (byte b2 : bArr) {
            i_renamed = (i_renamed << 8) | (b2 & 255);
            i2 += 8;
            while (i2 >= 8) {
                node = node.children[(i_renamed >>> (i2 - 8)) & 255];
        ...
        while (i2 > 0) {
            okhttp3.internal.http2.Huffman.Node node2 = node.children[(i_renamed << (8 - i2)) & 255];
    ...
    private void buildTree() {
        ...
        while (true) {
            ...
            addCode(i_renamed, CODES[i_renamed], bArr[i_renamed]);
            i_renamed++;
        }
    }

    private void addCode(int i_renamed, int i2, byte b2) {
        okhttp3.internal.http2.Huffman.Node node = new okhttp3.internal.http2.Huffman.Node(i_renamed, b2);
        okhttp3.internal.http2.Huffman.Node node2 = this.root;
        while (b2 > 8) {
            b2 = (byte) (b2 - 8);
            int i3 = (i2 >>> b2) & 255;
            ...
            if (node2.children[i3] == null) {
                node2.children[i3] = new okhttp3.internal.http2.Huffman.Node();
            }
    ...
    private static final class Node {
        ...
        Node() {
            this.children = new okhttp3.internal.http2.Huffman.Node[256];
            this.symbol = 0;
            this.terminalBits = 0;
        }

        Node(int i_renamed, int i2) {
            this.children = null;
            this.symbol = i_renamed;
            int i3 = i2 & 7;
